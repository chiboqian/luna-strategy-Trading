#!/usr/bin/env python3
"""
Runs a weekly backtest for a given strategy.
Opens position on the first business day of the week (usually Monday).
Closes position on the last business day of the week (usually Friday).

Usage:
    python util/run_weekly_backtest.py --start-date 2023-01-01 --end-date 2023-06-30 \
        --strategy synthetic_long --symbol SPY --parquet data/options.parquet \
        --output-dir results/spy_weekly --strategy-args "--days 30"
"""
...
def main():
    ...
    try:
        schedule = get_weekly_schedule(args.start_date, args.end_date)
    except Exception as e:
