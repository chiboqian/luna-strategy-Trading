...
logger = logging.getLogger("LongPut")
...
def main():
    ...
    parser.add_argument("--log-file", help="Log file name (default: long_put.log)")
    
    args = parser.parse_args()

    setup_logging(args.log_dir, args.log_file, default_dir='trading_logs/options', default_file='long_put.log')
    ...
    if not args.json:
        logger.info(f"--- Long Put Analysis ---")
        logger.info(f"Expiration: {selected_exp}")
        logger.info(f"Strike:     ${float(put_contract['strike_price']):.2f}")
